...
class MyQueue:
    ...
    def pop(self) -> int:
        self.move()
        return self.reverse_res.pop()


    def peek(self) -> int:
        self.move()
        return self.reverse_res[-1]

    def empty(self) -> bool:
        return (not self.res) and (not self.reverse_res)
        
    def move(self):
        if not self.reverse_res:
            while self.res:
                self.reverse_res.append(self.res.pop())
